...
// TestAutoRenameNotifiesAttachedClients proves the auto-rename path (here driven
// by a UserPromptSubmit hook) pushes a fresh session list to attached CLI clients
// via broadcastSessList, so the tab strip updates immediately instead of showing
// the stale name. We register a fake list sink (the same registry the attach loop
// uses) and assert it fires exactly once for the single first-prompt rename.
func TestAutoRenameNotifiesAttachedClients(t *testing.T) {
	d, repo := startTestDaemon(t)
	defer d.Stop()
	...
	prompt := "wire up the heartbeat goroutine"
	raw := `{"hook_event_name":"UserPromptSubmit","session_id":"` + s.ID + `","prompt":"` + prompt + `"}`
	if err := SendHook(repo, []byte(raw)); err != nil {
		t.Fatalf("SendHook: %v", err)
	}
	...
	// A second prompt must NOT re-push: ApplyAutoName is idempotent, so no rename
	// occurs and the tab strip is not needlessly refreshed.
	raw2 := `{"hook_event_name":"UserPromptSubmit","session_id":"` + s.ID + `","prompt":"a different second prompt"}`
	if err := SendHook(repo, []byte(raw2)); err != nil {
		t.Fatalf("SendHook 2: %v", err)
	}
	// Let the second hook drain through the same single-threaded ingest path as a
	// guaranteed-processed marker hook, then assert the count stayed at 1.
	stop := `{"hook_event_name":"Stop","session_id":"` + s.ID + `"}`
	if err := SendHook(repo, []byte(stop)); err != nil {
		t.Fatalf("SendHook stop: %v", err)
	}
	snapshot := collectEvents(d, "drain")
	waitFor(t, func() bool {
		for _, env := range snapshot() {
			if env.Event.Kind == event.KindStatusChange && env.Event.SessionID == s.ID {
				return true
			}
		}
		return false
	...
	mu.Unlock()
	if got != 1 {
		t.Fatalf("expected exactly 1 tab-strip push (first prompt only), got %d", got)
	}
}
