...
type State struct {
	...
	// scrollback retains lines evicted off the top of the default (non-alt)
	// screen by scrollUp, oldest-first, capped at maxScrollback (oldest dropped).
	// Upstream vt10x discarded these; the claude+ fork keeps them so the
	// compositor can offer a scrollback viewport.
	scrollback []line
}

// maxScrollback bounds the retained history ring (in lines).
...
func (t *State) scrollUp(orig, n int) {
	n = clamp(n, 0, t.bottom-orig+1)
	// When the full default (non-alt) region scrolls up from its top, the n top
	// lines are about to be evicted off-screen. Snapshot them into history
	// (oldest-first, capped) before clear() blanks them — upstream just dropped
	// them. Alt screen (e.g. a fullscreen TUI) is excluded: it has no scrollback.
	// The region must span the FULL screen (t.bottom == t.rows-1): a partial
	// top-pinned scroll region (DECSTBM, e.g. a fixed footer) scrolls app content
	// within the region, not real top-of-screen history, so it must not be retained.
	if orig == 0 && t.top == 0 && t.bottom == t.rows-1 && t.mode&ModeAltScreen == 0 {
		for i := 0; i < n; i++ {
			cp := make(line, len(t.lines[i]))
			copy(cp, t.lines[i])
			t.scrollback = append(t.scrollback, cp)
		}
...
// Scrollback returns the lines evicted off the top of the default screen,
// oldest-first. Returned glyph slices are copies safe to read after Unlock.
func (t *State) Scrollback() [][]Glyph {
	out := make([][]Glyph, len(t.scrollback))
	for i, ln := range t.scrollback {
