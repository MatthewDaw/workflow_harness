...
// agents, and the humanlayer-approvals MCP server into the project's
// enabledSkills / enabledAgents / enabledMcpServers. Idempotent (set-union).
//
...
if (!existsSync(path.join(backendDist, 'db', 'keys.js'))) {
  console.error(`[optin-hl] missing ${backendDist}/db/keys.js — run \`npm run build -w @harness/backend\` first.`);
  process.exit(1);
}
const { projectKey, scopePartition } = await import(pathToFileURL(path.join(backendDist, 'db', 'keys.js')).href);
...
const bundle = await doc.send(new GetCommand({ TableName: TABLE, Key: { PK: orgPart, SK: `SKILL#${BUNDLE}` } }));
const memberSkills = bundle.Item?.members ?? [];
if (memberSkills.length === 0) { console.error(`[optin-hl] bundle '${BUNDLE}' not found / empty in org#${ORG}`); process.exit(1); }
const agentNames = await queryNames('AGENT#');
const mcpNames = await queryNames('MCPSERVER#', (m) => m.name === 'humanlayer-approvals');
...
if (!project) { console.error(`[optin-hl] project '${PROJECT_ID}' not found`); process.exit(1); }
