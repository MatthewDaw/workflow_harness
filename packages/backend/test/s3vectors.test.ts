...
function vec(key: string, org = 'acme'): VectorItem {
  return { key, vector: Array.from({ length: 1536 }, () => 0.1), metadata: { org } };
}
...
describe('S3Vectors.queryTopK', () => {
  it('passes the org filter and returns ≤k hits sorted by score, dropping sub-floor', async () => {
    ...
    const hits = await store.queryTopK('skills', Array(1536).fill(0.1), 5, {
      orgFilter: 'acme',
      floor: 0.5,
    });
  ...
  it('caps results at k even when more come back', async () => {
    ...
    const hits = await store.queryTopK('skills', Array(1536).fill(0.1), 2);
  ...
  it('omits the filter when no orgFilter is given', async () => {
    ...
    await store.queryTopK('skills', Array(1536).fill(0.1), 3);
    const call = s3vMock.commandCalls(QueryVectorsCommand)[0]!.args[0].input;
  ...
  it('propagates a query error (does not swallow)', async () => {
    ...
    await expect(store.queryTopK('skills', Array(1536).fill(0.1), 3)).rejects.toThrow(
...
describe('S3Vectors.queryTopK — embedding version guard (U5)', () => {
  it('raises when an index hit is tagged with a different embeddingVersion', async () => {
    ...
    await expect(
      store.queryTopK('skills', Array(1536).fill(0.1), 5, { expectVersion: 'v2' }),
  ...
  it('defaults the expected version to the env-configured active version', async () => {
    const prev = process.env.OPENROUTER_EMBEDDING_VERSION;
    process.env.OPENROUTER_EMBEDDING_VERSION = 'v9';
    try {
      s3vMock.on(QueryVectorsCommand).resolves({
        distanceMetric: 'cosine',
        vectors: [
      ...
      // No expectVersion → falls back to OPENROUTER_EMBEDDING_VERSION (v9) ≠ v8 → raises.
      await expect(store.queryTopK('skills', Array(1536).fill(0.1), 5)).rejects.toBeInstanceOf(
        EmbeddingVersionMismatchError,
      );
    } finally {
      if (prev === undefined) delete process.env.OPENROUTER_EMBEDDING_VERSION;
      else process.env.OPENROUTER_EMBEDDING_VERSION = prev;
    }
  });
});
