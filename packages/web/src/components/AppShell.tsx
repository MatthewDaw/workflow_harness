      ...
      <div className="mx-auto max-w-[1180px] px-7 pb-24 pt-6">
        <div className="hq-frame overflow-hidden">
          <nav className="hq-appnav" aria-label="Primary">
            <span className="mr-3.5 font-bold tracking-wide">⌗ Command HQ</span>
            {NAV.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
            ...
            <span className="ml-auto flex items-center gap-2.5 text-xs text-faint">
              <span className="hq-dot hq-dot-live" />
              {liveCount} live ·{' '}
              <button
                type="button"
                onClick={() => void signOut()}
                className="cursor-pointer text-mut hover:text-ink"
                title="Sign out"
              >
                @{user?.username ?? 'me'}
              </button>
