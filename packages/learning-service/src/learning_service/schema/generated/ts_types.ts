...
export const skillKey = (org: string | number, baseName: string | number): { PK: string; SK: string } => ({
  PK: `SCOPE#org#${org}`,
  SK: `SKILL#${baseName}`,
});

/** Immutable revision snapshot. rev is zero-padded to PAD_WIDTHS['rev']. */
export const revisionKey = (org: string | number, variantId: string | number, rev: string | number): { PK: string; SK: string } => ({
  PK: `SCOPE#org#${org}`,
  SK: `SKILL#${variantId}#r${_pad(Number(rev), 12)}`,
});

/** Per-baseName org-wide TRUE pointer (which variant+rev is the default). */
export const truePointerKey = (org: string | number, baseName: string | number): { PK: string; SK: string } => ({
  PK: `SCOPE#org#${org}`,
  SK: `SKILL#${baseName}#TRUE`,
});

/** A candidate/folded insight co-located with its skill family. */
export const ideaKey = (org: string | number, skillBaseName: string | number, ideaId: string | number): { PK: string; SK: string } => ({
  PK: `SCOPE#org#${org}`,
  SK: `IDEA#${skillBaseName}#${ideaId}`,
});

/** One contribution (a merged PR or authored source) under its idea. */
export const ideaSourceKey = (org: string | number, ideaId: string | number, sourceId: string | number): { PK: string; SK: string } => ({
  PK: `SCOPE#org#${org}`,
  SK: `IDEASRC#${ideaId}#${sourceId}`,
});

/** Before→after golden regression case captured at fold time. */
export const goldenCaseKey = (org: string | number, skillBaseName: string | number, caseId: string | number): { PK: string; SK: string } => ({
  PK: `SCOPE#org#${org}`,
  SK: `IDEAGOLD#${skillBaseName}#${caseId}`,
});

/** Code-anchor index entry (file, symbol) → ideaId for the locality join. */
export const anchorKey = (org: string | number, ownerRepo: string | number, file: string | number, symbol: string | number, ideaId: string | number): { PK: string; SK: string } => ({
  PK: `SCOPE#org#${org}`,
  SK: `ANCHOR#${ownerRepo}#${file}#${symbol}#${ideaId}`,
});

/** Org-scoped idempotency cursor — marks a PR as already ingested. */
export const processedPrKey = (org: string | number, ownerRepo: string | number, prNumber: string | number): { PK: string; SK: string } => ({
  PK: `SCOPE#org#${org}`,
...
export interface TruePointerRecord {
...
export interface IdeaSourceRecord {
