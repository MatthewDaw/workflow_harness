...
### KTD1 — MCP server is a *structured* catalog record, not a markdown body
...
type McpServer =
  | { name; scope; transport: 'stdio'; command: string; args: string[]; env: Record<string,string>; createdBy? }
  | { name; scope; transport: 'http' | 'sse'; url: string; headers: Record<string,string>; createdBy? }
```
