...
**Location:** `C:\Users\mattd\AppData\Roaming\npm\node_modules\humanlayer\.claude\` (npm `humanlayer@0.17.2-npm`). Paths below are repo-relative _targets_; the source is this read-only package directory.
...
`skillSchema` (`packages/shared/src/dto.ts:207-237`) has **no `model` field**, so the `model: opus` pin on planning/research commands is dropped — those skills run at the session's model. `agentSchema` (`dto.ts:182-204`) **does** carry `model` (and `tools`), and per KTD8 those fields now _render_ into the materialized subagent's frontmatter rather than being stored-but-discarded — so the 6 subagents keep AND apply their `sonnet` pin and declared `tools`. This is an accepted fidelity loss for the command (skill) layer only (see Risks R2); the workflow still functions, it just isn't model-pinned per skill.
...
### U2. Vendor the 6 subagents as repo-managed agent files _(DONE)_
...
**Approach:** The 6 subagents are **vendored into `.claude/agents/`** (repo root, alongside `.claude/skills/`) verbatim, preserving `name` + `description` + `tools` + `model` frontmatter and body. Kebab names are kept (KTD2) — the imported skills reference these exact names. This directory is the source the dedicated agent seeder reads in U3. _(Status: all 6 files present at `.claude/agents/_.md`.)*
...
- Containment: a _different_ org's catalog is unchanged — the set did not propagate (it is absent from `bundles.json`, so `seed-all-orgs` won't pick it up).
...
### U5. Approval directive in high-stakes skills _(DONE)_
...
### U6. Remove the compound-engineering installer skill + scrub seed comments _(DONE)_
...
3. **U2** depends on U8 (vendors agent files against the corrected record/render contract). _(DONE)_
...
5. **U4** depends on U8 + U1 + U3 (bundle references skills; the scoped registration consumes the imported skill files). _(DONE: 27 skills + bundle in `org#test org`.)_
6. **U5** depends on U1 (edits skill bodies). _(DONE)_
...
## Addendum — post-landing fidelity pass (2026-06-07)

An audit of the upstream `humanlayer@0.17.2-npm` package against the landed integration found the _mechanical_ import complete and faithful (27 skills byte-identical bodies, 6 agents intact, U8 render + hash-parity, approval directives, `compound-engineering` removed). Two HumanLayer learnings were **not** captured by the original plan and were added in this pass:

### U10 — `contact_human`: the second HITL channel

The plan integrated only `claude_approvals` (gate a permission the agent is about to use). HumanLayer's headline capability is the _outbound_ channel — `humanlayer mcp serve` exposing `contact_human` (Slack/email/web), letting an agent **proactively reach a human** for a decision mid-run. This is most valuable in autonomous/headless runs where no human is at the terminal.

- **Files:** `infra/scripts/register-humanlayer-contact-mcp.mjs` (new; sibling of the approvals registrar — stdio, `command: humanlayer`, `args: ['mcp','serve']`, `env: {}`, single-org scoped, NOT org-wide). `optin-humanlayer.mjs` now enables both HL MCP servers. An "Ask a human" directive (mirroring the approval directive's graceful-degradation phrasing) is appended to the autonomous-loop skills `ralph_impl`, `ralph_plan`, `ralph_research`, `founder_mode`.
- **Secrets:** `contact_human` routing needs `HUMANLAYER_API_KEY` + a channel (`HUMANLAYER_SLACK_CHANNEL` / `HUMANLAYER_EMAIL_ADDRESS`); with none, HumanLayer falls back to its web UI. The catalog record stores `env: {}` (env values are plaintext in DynamoDB) — the secret/channel is supplied where the daemon runs, not baked into the record.

### U11 — extended-thinking ceiling (the achievable half of KTD3's model-fidelity loss)

Upstream `.claude/settings.json` sets `MAX_THINKING_TOKENS: 32000`, and the planning/research command **bodies** (imported verbatim) already contain the escalation magic words (`ultrathink`, `think deeply`). The wrapper now defaults `MAX_THINKING_TOKENS=32000` on the inner-claude launch (`wrapper/internal/pty/session.go` `DefaultSpawn`, **no-clobber** — an existing export wins), so `ultrathink` can spend its full budget. Tests: `wrapper/internal/pty/thinking_test.go`.

**On the dropped `model: opus` pins (KTD3 / R2 — now resolved as structurally unrecoverable at the skill layer):** Claude Code **skills (`SKILL.md`) have no `model:` field** — only slash-commands and subagents do. Because the wrapper imports commands as _skills_ (KTD1, no `commands/` concept), the `opus` pin on the 8 planning/research commands cannot be expressed on the materialized skill no matter what the catalog stores; a skill always runs at the session model. The thinking-budget capture (U11) is therefore the achievable substitute for "reason deeply on planning." The only ways to truly pin opus are (a) run the whole session on opus, or (b) re-import the planning commands as **agents** (which do carry `model`) — a deliberate architecture change left as future work, not a bug in this integration.
